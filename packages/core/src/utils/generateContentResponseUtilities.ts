...
export function extractTextAndErrorFromResponse(
  response: GenerateContentResponse,
): { text: string | undefined; errorText: string | undefined } {
  const fullText = getResponseText(response);
  if (!fullText) {
    return { text: undefined, errorText: undefined };
  }
  const errorMatch = fullText.match(/<ERROR>(.*?)<\/ERROR>/s);
  const errorText = errorMatch ? errorMatch[1].trim() : undefined;
  const textWithoutError = fullText.replace(/<ERROR>.*?<\/ERROR>/s, '').trim();
  return {
    text: textWithoutError === '' ? undefined : textWithoutError,
    errorText,
  };
}
